...
function extended_page_checker($url, $text, $notext, $user = "", $pass = "", $timeout = -1) // $text and $notext are arrays in this instance
{
	...
	if ($ctr < 0)
		return $ctr; // negative number (-1) failed to open
		...
		foreach ($text as $findthis) {
			if ($findthis != "")
				if (strpos($output, $findthis) === false)
					return -2; // text to be found not found
		}
	}
	if (is_array($notext)) {
		foreach ($notext as $donotfindthis) {
			if ($donotfindthis != "")
				if (strpos($output, $donotfindthis) !== false)
					return -3; // text not to find found
		}
	}
...
class Advanced_Pagecheck_Test extends FreeNATS_Local_Test
{
	...
	{
		if ($result > 0)
			return 0; // FreeNATS passed (0) flag if > 0
...
$NATS->Tests->Register(
	"advpagecheck",
	// the internal simple test name (must not conflict with anything else)
	"Advanced_Pagecheck_Test",
	// the class name (above)
	$params,
	// parameters (blank for now)
	"Web Content Test",
	// the display name of the test in the interface
	3,
