...
if (isset($NATS)) {
	...
	function detail_alert_handler($data)
	{
		...
		$detail_alert_config = array(
			...
			"status_on_open" => true,
			// on an alert open include node status
			"status_on_close" => true,
			// same for close

			"summary_on_open" => true,
			// on open show summary of other alerts
		...
		if ($data['event'] == "alert_action") {
			if ($data['name'] == $detail_alert_config["trigger"])
				return true; // clear
			else
				return false; // propgate
		...
		if (
			($open && $detail_alert_config['status_on_open']) ||
		...
		if (
			($open && $detail_alert_config['summary_on_open']) ||
