...
function is_x_in_period($timex, $period)
{
	...
	if ($period['dayofmonth'] > 0)
		if ($period['dayofmonth'] != $day)
			return false;

	// check against start/finish times 

	if ($period['starthour'] > $hour)
		return false; // not yet the hour
	if (($period['starthour'] == $hour) && ($period['startmin'] > $min))
		return false; // hour but not yet the minute

	if ($period['finishhour'] < $hour)
		return false; // past the finish hour
	if (($period['finishhour'] == $hour) && ($period['finishmin'] < $min))
		return false; // finish hour equal but mins past
