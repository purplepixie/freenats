...
class TFreeNATS
{
	...
	function Start()
	{
		...
		if (isset($_COOKIE['fn_lang']) && ($_COOKIE['fn_lang'] != ""))
			$l = $_COOKIE['fn_lang'];
		else
			$l = $this->Cfg->Get("site.language", "");
		$this->Lang->Load($l);
	...
	{
		global $NATS_Session;
		if ((isset($NATS_Session)) && ($NATS_Session->auth))
			$username = $NATS_Session->username;
		else
			$username = "";
		$l = $this->Cfg->Get("log.level");
		//echo "** $l **\n";
		if ($l == "")
			$l = 10; // debug logging if no variable
		if ($l < $loglevel)
			return false;
		if (strlen($logevent) > 249)
			$logevent = substr($logevent, 0, 245) . "...";
	...
	function AlertAction($nodeid, $alertlevel, $change, $alerttext)
	{
		...
		if (trim($alerttext) == "")
			return false; // no text so a bogus event i.e. -1 to 0
	...
	function ActionFlush()
	{
		...
		while ($row = $this->DB->Fetch_Array($r)) {
			...
			if ($row['atype'] == "email") {
				...
				if ($row['esubject'] == 0)
					$sub = "";
				elseif ($row['esubject'] == 1)
					$sub = $this->Cfg->Get("alert.subject.short", "FreeNATS Alert");
				//nodeid
				elseif ($row['esubject'] == 3) {
					...
					$sub = $this->Cfg->Get("alert.subject.long", "** FreeNATS Alert **");
				$body = "";
				if ($row['etype'] == 0)
					$body = $row['mdata'];
				else {
				...
				foreach ($tolist as $tourl) {
					if ($doalert) {
						...
						if ($fp > 0)
							...
							$this->Event("URL Alert Failed " . $url, 1, "Flush", "URL");
	...
	function SetAlerts($nodeid, $alertlevel, $alerts = "")
	{
		...
		$r = $this->DB->Query($q);
		if ($row = $this->DB->Fetch_Array($r)) { // yes there is
	...
	function GetNode($nodeid)
	{
		...
		{
			if ($row['nodename'] != "")
				$row['name'] = $row['nodename']; // make a "nice" name for it
			else
				$row['name'] = $row['nodeid'];
	...
	function InvalidateTest($testid, $rightnow = false)
	{
		...
		if ($class == "L")
			$q .= "fnlocaltest WHERE localtestid=";
		// other ones here
	...
	{ // nodeid + takes a TNodeXML Object
		...
		while ($row = $this->DB->Fetch_Array($r)) {
			...
			if (isset($xmlobj->Catalogue[$row['testtype']])) { // this test is in the DB and catalogue
				$tests[$row['testtype']]['incat'] = true;
				if ($row['testenabled'] == 1) // it is enabled - so we test it
				{
					if ($row['simpleeval'] == 1)
						$level = $xmlobj->Catalogue[$row['testtype']]['ALERTLEVEL']; // use provided level
					else
				...
				if ($row['testenabled'] == 1) { // enabled so shown in lists etc
		...
		if (isset($this->EventHandlers[$event])) // handler(s) exist
		{
			for ($a = 0; $a < count($this->EventHandlers[$event]); $a++) {
				if (function_exists($this->EventHandlers[$event][$a])) {
					...
					if ($this->EventHandlers[$event][$a]($data))
						$res = true; // persist true only
				} else {
	...
	{
		if (!$this->isUserRestricted($username))
			return true;
