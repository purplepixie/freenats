...
function lText($test)
{
	...
	switch ($test) {
		...
		case "wtime":
			return $NATS->Lang->Item("web.time");
		case "ICMP":
			return $NATS->Lang->Item("node.ping");
		case "ping":
		...
		default:
			if (isset($NATS)) {
				if (isset($NATS->Tests->QuickList[$test]))
					return $NATS->Tests->QuickList[$test];
			}
			return $test;
	}
}
...
function oText($val)
{
	...
	switch ($val) {
		case -1:
			if ($n)
				return $NATS->Cfg->Get("site.text.untested", $NATS->Lang->Item("untested"));
			return $NATS->Lang->Item("untested");
		case 0:
			if ($n)
				return $NATS->Cfg->Get("site.text.passed", $NATS->Lang->Item("passed"));
			return $NATS->Lang->Item("passed");
		case 1:
			if ($n)
				return $NATS->Cfg->Get("site.text.warning", $NATS->Lang->Item("warning"));
			return $NATS->Lang->Item("warning");
		case 2:
			if ($n)
				return $NATS->Cfg->Get("site.text.failed", $NATS->Lang->Item("failed"));
			return $NATS->Lang->Item("failed");
		default:
			if ($n)
				return $NATS->Cfg->Get("site.text.unknown", $NATS->Lang->Item("unknown"));
