...
function aat_etype($type)
{
	...
	switch ($type) {
		case 0:
			return $NATS->Lang->Item("aae.short");
		case 1:
			return $NATS->Lang->Item("aae.long");
		default:
...
function aat_esub($type)
{
	...
	switch ($type) {
		case 0:
			return $NATS->Lang->Item("aae.sub.blank");
		case 1:
			return $NATS->Lang->Item("aae.sub.short");
		case 2:
			return $NATS->Lang->Item("aae.sub.long");
		default:
			return $NATS->Lang->Item("unknown");
	}
}
...
function aat_atype($type)
{
	...
	switch ($type) {
		case "":
		case "Disabled":
			return $NATS->Lang->Item("aatype.disabled");
		case "email":
			return $NATS->Lang->Item("aatype.email");
		case "url":
			return $NATS->Lang->Item("aatype.url");
		case "mqueue":
			return $NATS->Lang->Item("aatype.mqueue");
		default:
			return $NATS->Lang->Item("unknown") . " (" . $type . ")";
	}
}
...
if ($mode == "users") {
	...
	if (isset($_REQUEST['aaid'])) { // view/edit aaid
