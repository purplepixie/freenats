	...
	switch ($_REQUEST['query'][$a]) {
		...
		case "test":
			...
			if ($testdata) // got a valid response
			{
				...
				if ((isset($_REQUEST['param1'][$a])) && (isset($_REQUEST['param2'][$a]))) { // get data
					...
					if (($testdata['testrecord'] == 1) || ($testdata['testtype'] == "ICMP")) {
						...
						while ($row = $NATS->DB->Fetch_Array($r)) {
							switch ($row['alertlevel']) {
								case -1:
									$testdata['period.untested'] += $row['counter'];
									break;
								case 0:
									$testdata['period.passed'] += $row['counter'];
									break;
								case 1:
									$testdata['period.warning'] += $row['counter'];
									break;
								case 2:
									$testdata['period.failed'] += $row['counter'];
									break;
							}
