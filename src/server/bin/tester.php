...
if ($suspend != 1 || $forcerun) {
	...
	if ($runrow = $NATS->DB->Fetch_Array($cr)) { // yes there is a testrun session for this node(s)
	...
	while ($row = $NATS->DB->Fetch_Array($r)) {
		...
		if ($row['pingtest'] && $dotests) {
			...
			$fr = $NATS->DB->Query($fq);
			$ltid_icmp = "";
			if ($irow = $NATS->DB->Fetch_Array($fr)) { // exists
		...
		} else { // further ICMP bodge - update to -1 or do nothing if the test doesn't exist
		...
		if (($dotests && ($row['nsenabled'] == 1) && ($row['nspullenabled'] == 1)) ||		// pull and tests are on
			(($row['nsenabled'] == 1) && ($row['nspushenabled'] == 1))
		)	// or pushed
		{
	...
	// in here for now...
	$NATS->ActionFlush();
} // end of suspend check
else {
	// Suspended
	db("NATS Testing Suspended");
}
